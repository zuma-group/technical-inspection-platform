        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Location: <strong>{equipment.location}</strong> • Hours:{" "}
              <strong>{equipment.hoursUsed}</strong>
            </p>
          </div>
          <div className="flex gap-3">
            {equipment.inspections.some(
              (insp) => insp.status === "IN_PROGRESS"
            ) ? (
              <button
      ...
      <div className="card">
        ...
        {equipment.inspections.length === 0 ? (
          <p className="text-gray-600">
            No previous inspections for this equipment.
          </p>
        ) : (
          <>
            {/* Desktop Table View */}
                          ...
                          >
                            ...
                            {insp.status === "IN_PROGRESS" && (
                              <Link href={`/inspect/${equipment.id}`}>
                                <button className="btn btn-warning text-sm flex-shrink whitespace-nowrap">
                                  Continue Inspection
                                </button>
                              </Link>
                            )}
                            <Link href={`/api/inspections/${insp.id}/pdf`}>
                              <button className="btn btn-primary text-sm flex-shrink whitespace-nowrap">
                                Download PDF
                              </button>
                            </Link>
                          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-row gap-2 w-full">
                      <Link href={`/inspections/${insp.id}`} className="flex-1">
                      ...
                      {insp.status === "IN_PROGRESS" && (
                        <Link
                          href={`/inspect/${equipment.id}`}
                          className="flex-1"
                        >
                          <button className="btn btn-warning text-sm flex-1 w-full ">
                            Continue
                          </button>
                        </Link>
                      )}
                      <Link
                        ...
                        className="flex-1"
                      >
                        <button className="btn btn-primary text-sm flex-1 w-full ">
                          PDF
                        </button>
                      </Link>
                    </div>
